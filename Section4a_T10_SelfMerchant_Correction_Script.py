...
methodology_lines = [
    ...
    "  - Typology 10 identifier-strength check (Query 13, Section 4.a): before finalizing the NOT "
    "CONFIRMED call above, the schema was re-checked for identifiers stronger than DOB -- tax_id/ssn, "
    "a common login device_id, and shared e-mail domain, as raised in review. Confirmed columns: "
    "customers_kyc carries full_name, dob, ssn_last4 (4-digit partial only), email, document_id, "
    "document_type, doc_issue_country; beneficial_owners_json UBO objects carry ONLY name, country, "
    "dob, ownership_pct, pep, sanctions_match_score. None of the three stronger identifiers exist on "
    "the UBO side of the schema today, so none can be evaluated -- this is a documented data-model gap, "
    "not a missed analysis step, and is recorded as a specification for future KYB schema extension "
    "rather than simulated. A three-tier MATCH_TIER model is now used consistently: CONFIRMED "
    "(identical tax_id/device_id/email -- zero achievable population until the schema is extended), "
    "CORROBORATED (DOB match only -- 0 of 175 today, matching the finding above), and "
    "NAME_ONLY_UNCONFIRMED (the remaining 175 name-only collisions). The 175 NAME_ONLY_UNCONFIRMED "
    "cases remain OUT of the customer risk-scoring ranking and are retained solely in the T10 sheet / "
    "Query 13 output as an audit log.",
...
confirm_rows = [
    ...
    ("T10 Self-Merchant", "NOT CONFIRMED -- MATCH_TIER: 0 CONFIRMED (tax_id/device_id/email absent from "
                           "UBO schema -- future-spec, see calibration note), 0 CORROBORATED (DOB), "
                           "175 NAME_ONLY_UNCONFIRMED (audit log only, excluded from ranking)"),
