...
Part of the CloudWalk AML/CTF technical case submission. See
MANIFEST_AND_SIGNATURES.pdf (package root) for the SHA-256 checksum and
digital signature covering this file.
...
class EddParams:
    ...
    urgent_sanctions_score: float = 0.7             # urgency/risk-rating escalation tier
...
def run_edd(cust: pd.DataFrame, cards: pd.DataFrame,
    ...
    def urgency(row):
        ...
        if row["sanctions_match_score"] > params.urgent_sanctions_score or "E" in codes or "I" in codes or "J" in codes:
            return "URGENT"
    ...
    def corrected_risk_rating(row):
        codes = set(row["criteria_codes"].split(", ")) if row["criteria_codes"] else set()
        if (row["sanctions_match_score"] > params.urgent_sanctions_score
                or "E" in codes or "H" in codes or "B" in codes or "I" in codes
                or "J" in codes or "D" in codes):
            return "high"
    ...
    edd["corrected_risk_rating"] = edd.apply(corrected_risk_rating, axis=1)
    edd["docs_required"] = edd.apply(docs_required, axis=1)
...
# ===========================================================================
# INFORMATIVE, SCHEMA-DEPENDENT SCREENS (T12 / T13)
# ===========================================================================
# These two screens mirror Section4a Queries 11 and 12 exactly (same logic,
# same default thresholds). They are deliberately NOT wired into
# TYPOLOGY_WEIGHTS / cust_score: the composite ranking, the SAR, the top-10
# casework, and the FTE/capacity model in Section4c are all already closed
# out against the 11-typology (T01-T11) baseline. Folding T12/T13 into the
# composite score now would re-base every one of those already-verified
# numbers without a full re-verification pass across every downstream
# document -- exactly the kind of cascading, unverified change the Fase 10/11
# audit trail flags as a bigger risk than leaving a documented gap open.
# They are exposed here as standalone, informative tables so the underlying
# data-availability question ("can we screen on this today?") has a real,
# runnable answer instead of a claim in a findings log.
import json as _json


def run_t12_ubo_jurisdiction_screening(merch: pd.DataFrame,
                                        params: TypologyParams = TypologyParams(),
                                        ubo_sanctions_threshold: float = 0.3,
                                        flag_countries: tuple = ("IR", "KP", "SY", "CU", "VE", "RU")
                                        ) -> pd.DataFrame:
    """T12 - UBO High-Risk Jurisdiction Screening (Section4a Query 11).

    Expands merchants_kyb.beneficial_owners_json (one row per UBO) and flags
    UBOs domiciled in a flagged jurisdiction, or with a UBO-level sanctions
    fuzzy-match score above threshold. Informative only -- not part of
    cust_score. Returns an empty DataFrame (with the expected columns) if
    beneficial_owners_json is absent or empty for every merchant, which is
    itself the finding worth surfacing rather than an error to hide.
    """
    flag_countries = set(c.upper() for c in flag_countries)
    rows = []
    if "beneficial_owners_json" in merch.columns:
        for _, mrow in merch.iterrows():
            raw = mrow.get("beneficial_owners_json")
            if raw is None or (isinstance(raw, float) and pd.isna(raw)):
                continue
            try:
                ubos = raw if isinstance(raw, list) else _json.loads(raw)
            except (TypeError, ValueError):
                continue
            for ubo in ubos or []:
                country = str(ubo.get("country", "")).upper()
                ownership = ubo.get("ownership_pct")
                sanctions_score = ubo.get("sanctions_match_score")
                pep = bool(ubo.get("pep", False))
                in_flagged = country in flag_countries
                fuzzy_hit = (sanctions_score is not None
                             and sanctions_score > ubo_sanctions_threshold)
                if not (in_flagged or fuzzy_hit):
                    continue
                if in_flagged and ownership is not None and ownership >= 50:
                    alert_type = "UBO_SANCTIONED_JURISDICTION_CONTROL_50PCT_RULE"
                elif in_flagged and pep:
                    alert_type = "UBO_SANCTIONED_JURISDICTION_AND_PEP"
                elif in_flagged:
                    alert_type = "UBO_FLAGGED_JURISDICTION"
                else:
                    alert_type = "UBO_SANCTIONS_FUZZY_MATCH"
                rows.append({
                    "alert_type": alert_type,
                    "merchant_id": mrow.get("merchant_id"),
                    "dba_name": mrow.get("dba_name"),
                    "primary_mcc": mrow.get("primary_mcc"),
                    "ubo_name": ubo.get("name"),
                    "ubo_country": country,
                    "ownership_pct": ownership,
                    "ubo_sanctions_match_score": sanctions_score,
                    "ubo_pep": pep,
                })
    cols = ["alert_type", "merchant_id", "dba_name", "primary_mcc", "ubo_name",
            "ubo_country", "ownership_pct", "ubo_sanctions_match_score", "ubo_pep"]
    df = pd.DataFrame(rows, columns=cols)
    if len(df):
        priority = {"UBO_SANCTIONED_JURISDICTION_CONTROL_50PCT_RULE": 0,
                    "UBO_SANCTIONED_JURISDICTION_AND_PEP": 1}
        df["_sort"] = df["alert_type"].map(lambda a: priority.get(a, 2))
        df = df.sort_values(["_sort", "ownership_pct"], ascending=[True, False]).drop(columns="_sort")
    return df.reset_index(drop=True)


def run_t13_prepaid_card_velocity(tx: pd.DataFrame, cards: pd.DataFrame,
                                   params: TypologyParams = TypologyParams(),
                                   min_txns_30d: int = 10,
                                   daily_limit: float = 1000.00,
                                   monthly_limit: float = 10000.00,
                                   window_days: int = 30) -> pd.DataFrame:
    """T13 - Prepaid Card Velocity in High-Risk MCCs (Section4a Query 12).

    Flags prepaid cards with unusually high 30-day transaction velocity at
    high-risk MCCs, or single-day/monthly spend above the FinCEN Prepaid
    Access Rule planning thresholds (31 CFR 1010). Informative only -- not
    part of cust_score, for the same reason as T12 above.
    """
    if "product" not in cards.columns:
        return pd.DataFrame(columns=["alert_type", "card_id", "customer_id",
                                      "high_risk_txn_count_30d", "total_amount_30d",
                                      "flagged_date", "daily_amount"])
    prepaid_ids = set(cards.loc[cards["product"] == "prepaid", "card_id"])
    if not prepaid_ids:
        return pd.DataFrame(columns=["alert_type", "card_id", "customer_id",
                                      "high_risk_txn_count_30d", "total_amount_30d",
                                      "flagged_date", "daily_amount"])
    ...
    window_start = dataset_max_ts - pd.Timedelta(days=window_days)
    recent = tx[(tx["card_id"].isin(prepaid_ids)) & (tx["txn_timestamp_utc"] >= window_start)].copy()
    recent["txn_date"] = recent["txn_timestamp_utc"].dt.date

    hr = set(HIGH_RISK_MCCS)
    velocity = recent.groupby(["card_id", "customer_id"]).apply(
        lambda g: pd.Series({
            "high_risk_txn_count_30d": g["mcc"].isin(hr).sum(),
            "total_amount_30d": g["amount_usd"].sum(),
        })
    ).reset_index()

    daily = recent.groupby(["card_id", "customer_id", "txn_date"])["amount_usd"].sum().reset_index()
    daily = daily[daily["amount_usd"] > daily_limit].rename(columns={"amount_usd": "daily_amount"})

    merged = velocity.merge(daily, on=["card_id", "customer_id"], how="left")

    def classify(row):
        if row["high_risk_txn_count_30d"] >= min_txns_30d:
            return "PREPAID_VELOCITY_HIGH_RISK_MCC"
        if row["total_amount_30d"] > monthly_limit:
            return "PREPAID_MONTHLY_LIMIT_EXCEEDED"
        return "PREPAID_DAILY_LIMIT_EXCEEDED"

    hit = merged[
        (merged["high_risk_txn_count_30d"] >= min_txns_30d)
        | (merged["total_amount_30d"] > monthly_limit)
        | (merged["daily_amount"].notna())
    ].copy()
    if len(hit):
        hit["alert_type"] = hit.apply(classify, axis=1)
        hit = hit.sort_values("high_risk_txn_count_30d", ascending=False)
    cols = ["alert_type", "card_id", "customer_id", "high_risk_txn_count_30d",
            "total_amount_30d", "txn_date", "daily_amount"]
    hit = hit.rename(columns={"txn_date": "flagged_date"})
    for c in cols:
